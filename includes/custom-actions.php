...
function atbdp_after_new_listing_button(): void
{
    do_action('atbdp_after_new_listing_button');
}


function atbdp_create_picvacyAndTerms_pages(): void
{
    ...
    if ($create_permission) {
        ...
        $pages = apply_filters('atbdp_create_picvacyAndTerms_pages', [
            'privacy_policy' => [
...
function atbdp_get_preview_button(): ?string
{
    $preview_enable = get_directorist_option('preview_enable', 1);
    if (!empty($preview_enable) && isset($_GET['redirect'])) {
...
function atbdp_is_extension_active(): bool
{
...
function atbdp_extend_extension_settings_submenus($default)
{
    if ( apply_filters( 'atbdp_extension_license_settings_init', atbdp_is_extension_active() ) ) {
        $array_license = [
            ...
            'controls' => apply_filters('atbdp_license_settings_controls', [
                [
                    'type' => 'notebox',
...
if(!function_exists('atbdp_country_code_to_name')){
    function atbdp_country_code_to_name(): array{
       return [
              'AF' => 'Afghanistan',
              'AL' => 'Albania',
              'DZ' => 'Algeria',
 ...
 */
function directorist_updated_post_meta_action( $meta_id, $object_id, $meta_key, $meta_value ): void {
