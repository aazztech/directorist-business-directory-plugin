...
class SetupWizard {
					...
					if ( 'category' == $key ) {
                        ...
                        if ( ! $term_exists ) { // @codingStandardsIgnoreLine.
							...
							if ( ! is_wp_error( $result ) ) {
								...
								update_term_meta( $term_id, '_directory_type', array( $directory_id ) );
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script(
			'directorist-test',
			DIRECTORIST_JS . 'openstreet-map.js',
			array(
				'jquery',
		...
		wp_localize_script(
			'directorist-admin-setup-wizard-script',
			'import_export_data',
			array(
	...
	protected function set_steps() {
		$this->steps = apply_filters(
			'directorist_admin_setup_wizard_steps',
			array(
				'introduction' => array(
	...
	public function directorist_step_two_save() {
		...
		if ( ! empty( $_post_data['active_gateways'] ) && in_array( 'paypal_gateway', $_post_data['active_gateways'] ) ) {
			directorist_download_plugin( array( 'url' => 'https://directorist.com/wp-content/uploads/2024/11/directorist-paypal.zip' ) );
	...
	public function directorist_setup_introduction() {
		...
		<div class="directorist-setup-wizard__content">
			<div class="directorist-setup-wizard__content__header">
				...
				<p class="directorist-setup-wizard__content__header__desc"><?php echo wp_kses( __( 'Select the directory type you’re building. Weather it’s a business directory, a classifieds platform, or something else, we’ve got you covered.', 'directorist' ), array( 'strong' => array() ) ); ?></p>
			</div>
