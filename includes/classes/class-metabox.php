...
class ATBDP_Metabox {
	...
	public function atbdp_dynamic_admin_listing_form(): void {
	...
	public function render_listing_taxonomies( $listing_id, $term_id, $taxonomy_id, $parent_id = 0 ): void {
		$args = [
			'hide_empty'   => 0,
			'hierarchical' => true,
			'parent'       => $parent_id
				...
				if ( in_array( $term_id, $directory_type_int, true ) ) { ?>
					<li id="<?php echo esc_attr( $taxonomy_id ); ?>-<?php echo esc_attr( $term->term_id ); ?>">
						<label class="selectit">
							<input value="<?php echo esc_attr( $term->term_id ); ?>" type="checkbox" name="tax_input[<?php echo esc_attr( $taxonomy_id ); ?>][]" id="in-<?php echo esc_attr( $taxonomy_id ); ?>-<?php echo esc_attr( $term->term_id ); ?>" <?php echo $checked === '' || $checked === '0' ? '' : esc_attr( $checked ); ?>>
	...
	public function render_listing_pop_taxonomies( $listing_id, $term_id, $taxonomy_id ): void {
		$args = [
			'hide_empty' => 0,
			'hierarchical' => false
		];
		...
		if( $terms ) {
			foreach( $terms as $term ) {
				...
				if( in_array( $term_id, $directory_type) ) { ?>
					<li id="popular-<?php echo esc_attr( $taxonomy_id ); ?>-<?php echo esc_attr( $term->term_id ); ?>" class="popular-category"><label class="selectit"><input value="<?php echo esc_attr( $term->term_id ); ?>" type="checkbox" id="in-popular-<?php echo esc_attr( $taxonomy_id ); ?>-<?php echo esc_attr( $term->term_id ); ?>" <?php echo $checked === '' || $checked === '0' ? '' : esc_attr( $checked ); ?>> <?php echo esc_html( $term->name ); ?></label></li>
	...
	public function listing_metabox( $post ): void {
	...
	public function render_listing_meta_fields( $type, $id ): void {
	...
	public function render_expire_date( $listing_id, $term_id ): void
	{
	...
	public function listing_form_info_meta( $post ): void {
     ...
     * @return mixed[]
     */
    public function build_form_data( $type ): array {
		$form_data              = [];
	 ...
	 * @since 5.4.0
	 */
	public function publish_atbdp_listings( $new_status, $old_status, $post ): void{
	 ...
	 * @param WP_Post $post
	 */
	public function post_submitbox_meta($post): void
	{

		if (ATBDP_POST_TYPE !=$post->post_type) {
            return;
        } // vail if it is not our post type
		// show expiration date and featured listing.
		...
		$e_d                    = empty($e_d) ? calc_listing_expiry_date( '', $expire_in_days ) : $e_d;
		$expiry_date            = atbdp_parse_mysql_date($e_d);
		...
		$data = ['f_active' => $f_active, 'never_expire' => $never_expire, 'expiry_date' => $expiry_date, 'featured' => $featured, 'listing_type' => $listing_type, 'listing_status' => $listing_status, 'default_expire_in_days' => $default_expire_in_days];

		if( apply_filters( 'directorist_before_featured_expire_metabox', true, $post ) ){
			ATBDP()->load_template('admin-templates/listing-form/expiration-featured-fields', ['data'=> $data]);
		}
	}
	 ...
	 */
	public function save_post_meta( $post_id, $post ): void {
		...
		$directory_id           = empty( $_POST['directory_type'] ) ? directorist_get_default_directory() : directorist_clean( wp_unslash( $_POST['directory_type'] ) );
		...
		if ( ! empty( $listing_categories ) && is_array( $listing_categories ) ) {
			foreach ( $listing_categories as $category ) {
				directorist_update_category_directory( $category, [ $directory_id ], true );
			}
		}

		if ( ! empty( $listing_locations ) && is_array( $listing_locations ) ) {
			foreach ( $listing_locations as $location ) {
				directorist_update_location_directory( $location, [ $directory_id ], true );
			}
		}
		...
		foreach ( $submission_form_fields as $key => $value ) {
			if ( 'image_upload' === $key ) {
				$meta_data['_listing_img']     = empty( $_POST['listing_img'] ) ? [] : (array) directorist_clean( wp_unslash( $_POST['listing_img'] ) );
		...
		$meta_data['_featured'] = (! is_fee_manager_active() || 'null' === $admin_plan) && ! empty( $_POST['featured'] );

		$should_never_expire = ! empty( $_POST['never_expire'] );
		$meta_data['_never_expire'] = $should_never_expire || ! $default_expiration ? true : null;

		//prepare expiry date, if we receive complete expire date from the submitted post, then use it, else use the default data
		$expire_date = empty( $_POST['exp_date'] ) ? [] : directorist_clean( wp_unslash( $_POST['exp_date'] ) );
		if ( ! is_empty_v( $expire_date ) && ! empty( $expire_date['aa'] ) ) {
			$expire_date = get_date_in_mysql_format( [
		...
		if ( (empty( $listing_status ) || 'expired' === $listing_status && 'private' === $listing_status) && ($expire_date > $current_date || $should_never_expire) ) {
			wp_update_post( [
					'ID'          => $post_id,
