...
class Popular_Listings extends \WP_Widget {
    ...
    {
        return ['title' => empty( $new_instance['title'] ) ? '' : sanitize_text_field( $new_instance['title'] ), 'pop_listing_num' => empty( $new_instance['pop_listing_num'] ) ? '' : sanitize_text_field( $new_instance['pop_listing_num'] ), 'single_only' => empty( $new_instance['single_only'] ) ? 0 : 1];
    }
	...
	public function widget( $args, $instance ): void {
		...
		$count = empty( $instance['pop_listing_num'] ) ? 5 : $instance['pop_listing_num'];
		$query = $this->popular_listings_query( $count );

		Helper::get_template( 'widgets/popular-listings', ['args' => $args, 'instance' => $instance, 'query' => $query] );
	...
	public function popular_listings_query( $count = 5 ) {
        ...
        } elseif ('average_rating' === $listing_popular_by) {
            $meta_queries['_rating'] = [
					'key'     => directorist_get_rating_field_meta_key(),
					'value'   => $average_review_for_popular,
					'type'    => 'NUMERIC',
