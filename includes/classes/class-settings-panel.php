...
if ( ! class_exists( 'ATBDP_Settings_Panel' ) ) {
	class ATBDP_Settings_Panel {
		...
		public $config            = array();
		public $default_form      = array();
		public $old_custom_fields = array();
		public $cetagory_options  = array();

		// run
		public function run() {
			add_action( 'directorist_installed', array( $this, 'update_init_options' ) );
			...
			add_action( 'wp_ajax_save_settings_data', array( $this, 'handle_save_settings_data_request' ) );
			add_action( 'wp_ajax_save_settings_data', array( $this, 'handle_save_settings_data_request' ) );
		...
		public function register_setting_fields( $fields = array() ) {
			$fields['script_debugging'] = array(
				'type'        => 'toggle',
			...
			$fields['single_listing_slug_with_directory_type'] = array(
				'type'    => 'toggle',
			...
			$fields['restore_default_settings'] = array(
				'type'         => 'restore',
				'label'        => 'Restore Default Settings',
				'button-label' => 'Restore',
				'restor-data'  => $this->get_simple_data_content( array( 'path' => 'directory/directory-settings.json' ) ),
			);

			$fields['enable_multi_directory'] = array(
				'type'                  => 'toggle',
				'label'                 => 'Enable Multi Directory',
				'value'                 => false,
				'confirm-before-change' => true,
				'confirmation-modal'    => array(
					'show-model-header' => false,
				),
				'data-on-change'        => array(
					'action' => 'updateData',
					'args'   => array( 'reload_after_save' => true ),
				),
				'componets'             => array(
					'link' => array(
			...
			$fields['listing_export_button'] = array(
				'type'                     => 'export-data',
...
SWBD;
			...
			// Marker Clustering
			$fields['marker_clustering'] = array(
				'type'    => 'toggle',
			...
			$fields['country_restriction'] = array(
				'type'    => 'toggle',
			...
			$fields['restricted_countries'] = array(
				'type'    => 'checkbox',
			...
			// Single Listings
			$fields['submission_confirmation'] = array(
				'type'  => 'toggle',
				'label' => __( 'Show Submission Confirmation', 'directorist' ),
				'value' => true,
			);

			$fields['pending_confirmation_msg'] = array(
				'type'    => 'textarea',
			...
			$fields['publish_confirmation_msg'] = array(
				'type'    => 'textarea',
		...
		public function get_simple_data_content( array $args = array() ) {
			$default = array(
				'path'        => '',
				'json_decode' => true,
			);
		...
		public function handle_save_settings_data_request() {
			...
			$field_list = ( ! empty( $_POST['field_list'] ) ) ? Directorist\Helper::maybe_json( sanitize_text_field( wp_unslash( $_POST['field_list'] ) ) ) : array();
...
Please remember that your order may be canceled if you do not make your payment within next 72 hours.';
			...
			$this->fields = apply_filters(
				...
				array(
					...
					'enable_featured_listing'              => array(
                        ...
                        <a href="%s" target="_blank">Watch how</a>',
								'directorist'
							),
					...
					'payment_thousand_separator'           => array(
						'type'        => 'text',
					...
					'payment_decimal_separator'            => array(
						'type'        => 'text',
					...
					'payment_currency_position'            => array(
					...
					'bank_transfer_description'            => array(
						'type'        => 'textarea',
					...
					'bank_transfer_instruction'            => array(
						'type'        => 'textarea',
					...
					// Secondary Button
					'button_secondary_example'             => array(
						'label'        => __( 'Button Example', 'directorist' ),
					...
					'button_secondary_bg_color'            => array(
						'label'   => __( 'Background Color', 'directorist' ),
						'type'    => 'color',
						'value'   => '#f2f3f5',
						'show-if' => array(
					...
					'popular_back_color'                   => array(
						'type'  => 'color',
						'label' => __( 'Background Color', 'directorist' ),
						'value' => '#f51957',
					),

					'new_back_color'                       => array(
						'type'  => 'color',
						'label' => __( 'Background Color', 'directorist' ),
						'value' => '#2C99FF',
					),
					...
					'delete_expired_listing_permanently'   => array(
					...
					'delete_expired_listings_after'        => array(
					...
					'listing_hide_top_search_bar'          => array(
						'type'    => 'toggle',
					...
					'listings_sidebar_filter_text'         => array(
						'type'    => 'text',
					...
					'listings_sidebar_reset_text'          => array(
						'type'    => 'text',
					...
					'preview_image_quality'                => array(
						...
						'description' => sprintf( __( 'Default: %1$s.<br/>If changed, regenerate thumbnails via <a href="%2$s" target="_blank">this</a> plugin for proper functionality.', 'directorist' ), $default_preview_size_text, 'https://wordpress.org/plugins/regenerate-thumbnails/' ),
					),
					...
					'restrict_single_listing_for_logged_in_user' => array(
						'type'  => 'toggle',
					...
					'dsiplay_slider_single_page'           => array(
						'type'        => 'toggle',
					...
					'single_slider_background_color'       => array(
						'type'    => 'color',
						'label'   => __( 'Background Color', 'directorist' ),
						'show-if' => array(
					...
					'average_review_for_popular'           => array(
					...
					'my_listing_tab'                       => array(
						'type'  => 'toggle',
						'label' => __( 'Display My Listing Tab', 'directorist' ),
						'value' => true,
					),
					'my_listing_tab_text'                  => array(
						'type'    => 'text',
					...
					'user_listings_pagination'             => array(
						'type'    => 'toggle',
					...
					'my_profile_tab'                       => array(
						'type'  => 'toggle',
						'label' => __( 'Display My Profile Tab', 'directorist' ),
						'value' => true,
					),
					'my_profile_tab_text'                  => array(
						'type'    => 'text',
					...
					'all_authors_description'              => array(
						'type'  => 'toggle',
						'label' => __( 'Display Description', 'directorist' ),
						'value' => true,
					),
					'all_authors_description_limit'        => array(
					...
					'all_authors_pagination'               => array(
						'type'  => 'toggle',
						'label' => __( 'Paginate All Authors ', 'directorist' ),
						'value' => true,
					),
					'all_authors_per_page'                 => array(
					...
					'show_popular_category'                => array(
						'type'  => 'toggle',
						'label' => __( 'Display Popular Categories', 'directorist' ),
						'value' => false,
					),

					'popular_cat_title'                    => array(
						'type'    => 'text',
					...
					'search_result_hide_top_search_bar'    => array(
						'type'    => 'toggle',
						'label'   => __( 'Hide Top Search Bar', 'directorist' ),
						'value'   => false,
						'show-if' => array(
					...
					'search_result_sidebar_filter_text'    => array(
						'type'    => 'text',
						'label'   => __( 'Filters Text', 'directorist' ),
						'value'   => __( 'Filters', 'directorist' ),
						'show-if' => array(
					...
					'search_result_filter_button_text'     => array(
						'type'    => 'text',
						'label'   => __( 'Filters Button Text', 'directorist' ),
						'value'   => __( 'Filters', 'directorist' ),
						'show-if' => array(
					...
					'display_search_result_listings_count' => array(
						'type'    => 'toggle',
						'label'   => __( 'Display Listings Count', 'directorist' ),
						'value'   => true,
						'show-if' => array(
					...
					'search_result_listing_title'          => array(
						'type'    => 'text',
						'label'   => __( 'Listings Count Text', 'directorist' ),
						'value'   => __( 'Items Found', 'directorist' ),
						'show-if' => array(
					...
					'sresult_sidebar_reset_text'           => array(
						'type'    => 'text',
						'label'   => __( 'Reset text', 'directorist' ),
						'value'   => __( 'Clear All', 'directorist' ),
						'show-if' => array(
					...
					'search_sortby_text'                   => array(
						'type'  => 'text',
						'label' => __( '"Sort By" Label', 'directorist' ),
						'value' => __( 'Sort By', 'directorist' ),
					),
					'search_sort_by_items'                 => array(
						'type'        => 'checkbox',
						'label'       => __( 'Sort Options', 'directorist' ),
						'description' => '',
						'value'       => array(
					...
					'search_result_filters_button_display' => array(
						'type'    => 'toggle',
						'label'   => __( 'Display Filters Button', 'directorist' ),
						'show-if' => array(
							'where'      => 'search_result_layout',
					...
					// seo settings
					'atbdp_enable_seo'                     => array(
						'type'  => 'toggle',
						'label' => __( 'Enable SEO', 'directorist' ),
						'value' => true,
					),
					'add_listing_page_meta_title'          => array(
						'type'        => 'text',
					...
					'meta_title_for_search_result'         => array(
					...
					'search_result_meta_title'             => array(
						'type'        => 'text',
					...
					'categories_column_number'             => array(
						'label'       => __( 'Number of  Columns', 'directorist' ),
					...
					'display_location_listing_count'       => array(
						'label' => __( 'Display Listing Count', 'directorist' ),
						'type'  => 'toggle',
						'value' => true,
					),
					'hide_empty_locations'                 => array(
					...
					'enable_email_verification'            => array(
					...
					'registration_privacy'                 => array(
						'label' => __( 'Enable', 'directorist' ),
						'type'  => 'toggle',
						'value' => true,
					),
					'registration_privacy_label'           => array(
						'type'    => 'text',
						'label'   => __( 'Label', 'directorist' ),
					...
					'registration_privacy_label_link'      => array(
						'type'    => 'text',
					...
					'regi_terms_condition'                 => array(
						'label' => __( 'Enable', 'directorist' ),
						'type'  => 'toggle',
						'value' => true,
					),
					'regi_terms_label'                     => array(
						'type'    => 'text',
						'label'   => __( 'Label', 'directorist' ),
					...
					'display_rememberme'                   => array(
						'label' => __( 'Enable', 'directorist' ),
						'type'  => 'toggle',
						'value' => true,
					),
					'log_rememberme'                       => array(
						'type'    => 'text',
						'label'   => __( 'Label', 'directorist' ),
					...
					'disable_email_notification'           => array(
					...
					'allow_email_header'                   => array(
						'label' => __( 'Email Header', 'directorist' ),
						'type'  => 'toggle',
						'value' => true,
					),
					'email_header_color'                   => array(
						'type'    => 'color',
					...
					'email_sub_to_expire_listing'          => array(
						'type'        => 'text',
						'label'       => __( 'Email Subject', 'directorist' ),
					...
					'email_tmpl_to_expire_listing'         => array(
						'type'        => 'textarea',
						'label'       => __( 'Email Body', 'directorist' ),
					...
					'email_sub_expired_listing'            => array(
						'type'        => 'text',
						'label'       => __( 'Email Subject', 'directorist' ),
					...
					'email_tmpl_expired_listing'           => array(
						'type'        => 'textarea',
						'label'       => __( 'Email Body', 'directorist' ),
					...
					'email_sub_to_renewal_listing'         => array(
						'type'        => 'text',
						'label'       => __( 'Email Subject', 'directorist' ),
					...
					'email_tmpl_to_renewal_listing'        => array(
						'type'        => 'textarea',
						'label'       => __( 'Email Body', 'directorist' ),
					...
					'email_tmpl_renewed_listing'           => array(
						'type'        => 'textarea',
						'label'       => __( 'Email Body', 'directorist' ),
					...
					'email_sub_deleted_listing'            => array(
						'type'        => 'text',
						'label'       => __( 'Email Subject', 'directorist' ),
					...
					'email_tmpl_deleted_listing'           => array(
						'type'        => 'textarea',
						'label'       => __( 'Email Body', 'directorist' ),
					...
					'email_sub_offline_new_order'          => array(
						'type'        => 'text',
						'label'       => __( 'Email Subject', 'directorist' ),
					...
					'email_tmpl_offline_new_order'         => array(
						'type'        => 'textarea',
						'label'       => __( 'Email Body', 'directorist' ),
					...
					'email_sub_completed_order'            => array(
						'type'        => 'text',
						'label'       => __( 'Email Subject', 'directorist' ),
					...
					'email_tmpl_completed_order'           => array(
						'type'        => 'textarea',
						'label'       => __( 'Email Body', 'directorist' ),
					...
					'email_sub_listing_contact_email'      => array(
						'type'        => 'text',
						'label'       => __( 'Email Subject', 'directorist' ),
					...
					'email_tmpl_listing_contact_email'     => array(
						'type'        => 'textarea',
						'label'       => __( 'Email Body', 'directorist' ),
					...
					'email_sub_registration_confirmation'  => array(
						'type'        => 'text',
						'label'       => __( 'Email Subject', 'directorist' ),
						'description' => __( 'Edit the subject for sending to the user when listing contact message send.', 'directorist' ),
						'value'       => __( 'Registration Confirmation!', 'directorist' ),
					),
					'email_tmpl_registration_confirmation' => array(
						'type'        => 'textarea',
						'label'       => __( 'Email Body', 'directorist' ),
					...
					// Email Verification
					'email_sub_email_verification'         => array(
						'type'        => 'text',
						'label'       => __( 'Email Subject', 'directorist' ),
						'description' => __( 'Edit the subject for sending to the user when listing contact message send.', 'directorist' ),
						'value'       => __( '[==NAME==] Verify Your Email', 'directorist' ),
					),
					'email_tmpl_email_verification'        => array(
						'type'        => 'textarea',
						'label'       => __( 'Email Body', 'directorist' ),
						'description' => __( 'Note: Use ==USER_PASSWORD== to show a temporary password when password field is disable from registration page', 'directorist' ),
						'value'       => __(
								...
								'general'              => array(
									...
									'sections' => apply_filters(
										...
										array(
											...
											'expired_listings_actions' => array(
		...
		public function add_menu_pages() {
			add_submenu_page(
				...
				'manage_options',
				'atbdp-settings',
				array( $this, 'menu_page_callback__settings_manager' ),
				12
			);
		}
