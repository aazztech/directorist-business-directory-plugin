...
class Multi_Directory_Manager {
	...
	public function __construct() {
		self::$migration = new Multi_Directory_Migration( array( 'multi_directory_manager' => $this ) );
	}

	// run
	public function run() {
		add_action( 'init', array( $this, 'register_directory_taxonomy' ) );
		add_action( 'init', array( $this, 'setup_migration' ) );

		if ( ! is_admin() ) {
			return;
		}

		add_filter( 'cptm_fields_before_update', array( $this, 'cptm_fields_before_update' ), 20, 1 );

		add_action( 'admin_menu', array( $this, 'add_menu_pages' ) );
		add_action( 'admin_post_delete_listing_type', array( $this, 'handle_delete_listing_type_request' ) );

		// Ajax
		add_action( 'wp_ajax_save_post_type_data', array( $this, 'save_post_type_data' ) );
		add_action( 'wp_ajax_save_imported_post_type_data', array( $this, 'save_imported_post_type_data' ) );
		add_action( 'wp_ajax_directorist_force_migrate', array( $this, 'handle_force_migration' ) );
		add_action( 'wp_ajax_directorist_directory_type_library', array( $this, 'directorist_directory_type_library' ) );
	}
	...
	public static function builder_data_backup( $term_id ) {
		...
		$existing_backup_data = get_option( 'directorist_builder_backup_data', array() );

		// Decode the JSON data if it exists
		$existing_backup_data = ! empty( $existing_backup_data ) ? json_decode( $existing_backup_data, true ) : array();

		if ( ! empty( $submission_form_fields ) && ! empty( $single_listings_contents ) && ! empty( $single_listing_header ) ) {
			$existing_backup_data[ $term_id ] = array(
	...
	public function directorist_directory_type_library() {
		...
		$installed = directorist_download_plugin( array( 'url' => 'https://downloads.wordpress.org/plugin/templatiq.zip' ) );
	...
	public function save_imported_post_type_data() {
		...
		// Validate file
		if ( empty( $json_file ) ) {
			$response['status']['status_log']['file_is_missing'] = array(
				'type'    => 'error',
		...
		if ( empty( $file_contents ) ) {
			$response['status']['status_log']['invalid_data'] = array(
				'type'    => 'error',
		...
		foreach ( $fields_group as $group_name => $group_fields ) {
			...
			foreach ( $group_fields as $field_index => $field_key ) {
				...
				if ( is_array( $field_key ) ) {
					$grouped_fields_value[ $field_index ] = array();
	...
	public function save_post_type_data() {
		...
		$fields     = array();
		$field_list = ! empty( $_POST['field_list'] ) ? directorist_maybe_json( wp_unslash( $_POST['field_list'] ) ) : array();
	...
	public function add_menu_pages() {
		...
		add_submenu_page(
			...
			'manage_options',
			$page_slug,
			array( $this, 'menu_page_callback__directory_types' ),
			5
		);
	}
		...
		foreach ( self::$config['fields_group'] as $group_key => $group_fields ) {
			if ( array_key_exists( $group_key, $all_term_meta ) ) {
				...
				foreach ( $group_fields as $field_index => $field_key ) {
					...
					if ( ! key_exists( $field_key, $group_value ) ) {
						continue; }
