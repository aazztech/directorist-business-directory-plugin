...
trait Multi_Directory_Helper {
	...
	public static function add_directory( array $args = array() ) {
		...
		$create_directory = array( 'term_id' => 0 );
	...
	public static function create_directory( array $args = array() ) {
		...
		if ( empty( $directory_name ) ) {
			$response['status']['status_log']['name_is_missing'] = array(
				'type'    => 'error',
		...
		if ( ! empty( $directory_name ) && $term = term_exists( $directory_name, 'atbdp_listing_types' ) ) {
			$response['status']['status_log']['term_exists'] = array(
				'type'    => 'error',
		...
		$response['status']['status_log']['term_created'] = array(
			'type'    => 'success',
	...
	public static function update_directory( array $args = array() ) {
		...
		if ( $has_invalid_term_id ) {
			$response['status']['status_log']['invalid_term_id'] = array(
				'type'    => 'error',
		...
		if ( $has_diffrent_name && term_exists( $directory_name, 'atbdp_listing_types' ) ) {
			$response['status']['status_log']['name_exists'] = array(
				'type'    => 'error',
				'message' => __( 'The name already exists', 'directorist' ),
			);
		...
		if ( ! empty( $directory_name ) ) {
			wp_update_term( $args['term_id'], 'atbdp_listing_types', array( 'name' => $directory_name ) );
		}
		...
		$response['status']['status_log']['term_updated'] = array(
			'type'    => 'success',
