 ...
 if ( ! class_exists( 'ATBDP_Tools' ) ) :
    class ATBDP_Tools
    {
        ...
        public function __construct()
        {
            ...
            add_action('admin_menu', [$this, 'add_tools_submenu'], 10);
            add_action('admin_init', [$this, 'atbdp_csv_import_controller']);
            add_action( 'init', [$this, 'prepare_data'] );
            add_action('wp_ajax_atbdp_import_listing', [$this, 'atbdp_import_listing']);
            add_action('wp_ajax_directorist_listing_type_form_fields', [$this, 'directorist_listing_type_form_fields']);
        }


        public function directorist_listing_type_form_fields(): void {
            ...
            ATBDP()->load_template( 'admin-templates/import-export/data-table', [ 'data' => csv_get_data( $file, false, $delimiter ), 'fields' => $this->get_importable_fields(), 'csv_file' => $file ] );
        ...
        public function atbdp_import_listing(): void {
            ...
            foreach ( $posts as $index => $post ) {
                    ...
                    $post_status = $post[ $listing_status ] ?? '';
					...
					$listing_id  = empty( $post['id'] ) ? 0 : absint( $post['id'] );
                    ...
                    if ( $tax_inputs ) {
                        foreach ( $tax_inputs as $taxonomy => $value ) {
                            ...
                            $terms = isset( $post[ $value ] ) && ! empty( $post[ $value ] ) ? explode( ',', $post[ $value ] ) : [];
                            if( $terms === [] ) {
                                continue;
                            }
                            ...
                            foreach ( $terms as $term ) {
								...
								if ( $taxonomy === ATBDP_CATEGORY || $taxonomy === ATBDP_LOCATION ) {
									directorist_update_term_directory( $term_id, [ $directory_type ], true );
								}
                    ...
                    if ( ! empty( $post['directory_type'] ) ) {
                        ...
                        $directory_type = ( empty( $directory_type_term ) ) ? $directory_type : $directory_type_term->term_id;
                    }
                    ...
                    $preview_url = $post[$preview_image] ?? '';
            ...
            $data['next_position'] = $position + $count;
		 ...
		 */
		public function get_or_create_term_id( $term, $taxonomy ): ?int {
        ...
        public static function atbdp_legacy_insert_attachment_from_url( string $file_url, $post_id ) {
		...
		public static function on_wp_handle_upload_prefilter( $file ) {
            ...
            } elseif (! in_array( $file['type'], $allowed_mimes, true )) {
        ...
        public function atbdp_csv_import_controller(): void {
			...
			add_filter( 'wp_handle_upload_prefilter', [ self::class, 'on_wp_handle_upload_prefilter' ] );

            $file = wp_import_handle_upload();

			remove_filter( 'wp_handle_upload_prefilter', [ self::class, 'on_wp_handle_upload_prefilter' ] );
        ...
        public function setup_fields( $directory = '' ): void {
        ...
        public function add_tools_submenu(): void {
			add_submenu_page(
				...
				'manage_options',
				'tools',
				[ $this, 'render_tools_submenu_page' ]
			);
        ...
        public function get_data_table( $file_path, $delimiter = ',' ): void{
        ...
        public function render_tools_submenu_page(): void {
         ...
         */
        public function importer_header_template( $return = false ): void {
            $template_data = [];
         ...
         * @return string $template
         */
        public function importer_header_nav_menu_item_template( array $template_data = [], $return = false ): void {
        ...
        public function get_header_nav_menu() {
            ...
            // Item - 1
            $nav_item                   = [];
            $nav_item['nav_item_class'] = $step ? ($step > 1 ? esc_attr('done') : '') : ( esc_attr('active'));
