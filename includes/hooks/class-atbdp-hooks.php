...
if ( ! class_exists( 'ATBDP_Hooks' ) ) :
	class ATBDP_Hooks {
		 ...
		 * @return void
		 */
		public static function add_hook( array $hook = array() ) {
			if ( class_exists( $hook['callback'] ) ) {
				if ( method_exists( $hook['callback'], 'run' ) ) {
					...
					if ( 'action' === $hook['type'] ) {
						add_action( $hook['name'], array( $callback, 'run' ), $priority, $accepted_args );
					}

					if ( 'filter' === $hook['type'] ) {
						add_filter( $hook['name'], array( $callback, 'run' ), $priority, $accepted_args );
					}
				}
			}
		}
