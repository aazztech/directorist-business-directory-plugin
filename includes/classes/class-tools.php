...
if ( ! class_exists( 'ATBDP_Tools' ) ) :
	class ATBDP_Tools {
		...
		public $importable_fields = array();
		...
		public function __construct() {
			...
			add_action( 'init', array( $this, 'prepare_data' ) );
		...
		public function atbdp_import_listing() {
			...
			$all_posts             = isset( $_POST['csv_file'] ) ? csv_get_data( directorist_clean( wp_unslash( $_POST['csv_file'] ) ), true, $delimiter ) : array();
			...
			$posts                 = ( ! empty( $all_posts ) ) ? array_slice( $all_posts, $position ) : array();
		...
		public function render_tools_submenu_page() {
			...
			ATBDP()->load_template( 'admin-templates/import-export/import-export', array( 'controller' => $this ) );
		}
		 ...
		 * @return string $template
		 */
		public function importer_header_nav_menu_item_template( $template_data = array(), $return = false ) {
