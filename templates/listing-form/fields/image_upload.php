...
$maximum                = empty( $data['max'] ) ? $data['max_image_limit'] : $data['max'];
...
$select_files_label     = empty( $data['select_files_label'] ) ? __( 'Select Files', 'directorist' ) : $data['select_files_label']; 
...
$img_upload_data = [
	'type'               => implode( ', ', $accepted_mime_types ),
	...
	<div class="ez-media-uploader directorist-image-upload" data-uploader="<?php echo esc_attr( $img_upload_data ); ?>">
		...
		<div class="ezmu-dictionary">
			...
			<span class="ezmu-dictionary-info-max-file-items">
				<?php echo empty($unlimited) ? ($limit > 1 ? esc_html__('Maximum __DT__ files are allowed', 'directorist') : esc_html__( 'Maximum __DT__ file is allowed', 'directorist' )) : ( esc_html__( 'Unlimited images with this plan!', 'directorist' ) ); ?>
			</span>
		</div>
	</div>
</div>
