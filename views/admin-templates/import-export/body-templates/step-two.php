...
// csv_from_builder
function csv_from_builder( $data = array() ) {
					...
					<div class="directory_type_wrapper">
						...
						if ( $csv_from_builder ) :
							...
							foreach ( $builder_posts[0] as $post_key => $post_value ) {
								?>
								<input type="hidden" class="atbdp_map_to" name="<?php echo esc_attr( $post_key ); ?>" value="<?php echo esc_attr( $post_key ); ?>">
								<?php
							}
						else :
	...
	<div class="csv-center">
		<div class="directorist-importer__importing" style="display: none;">
			<header>
				...
				<h2>
				<?php
				esc_html_e( 'Importing', 'directorist' );
				?>
					</h2>
				<p>
				<?php
				esc_html_e( 'Your listings are now being imported...', 'directorist' );
				?>
					</p>
			</header>
			<section>
