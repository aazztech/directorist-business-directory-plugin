...
class Multi_Directory_Migration {
	...
	public function migrate( array $args = array() ) {
		...
		if ( $add_directory['status']['success'] ) {
			...
			foreach ( $terms as $term ) {
				...
				if ( ! empty( $term_data ) ) {
					foreach ( $term_data as $data ) {
						update_term_meta( $data->term_id, '_directory_type', array( $add_directory['term_id'] ) );
					}
				}
			}
			...
			$listings_ids = ( ! is_wp_error( $listings ) ) ? wp_parse_id_list( $listings->posts ) : array();
	...
	public function get_fields_data() {
		...
		$form_fields_common_data = array( 'old_custom_fields' => $old_custom_fields );
		...
		$listings_card_common_data = array( 'listings_card_wedgets' => $listings_card_wedgets );
	...
	public function get_submission_form_fields_data( array $args = array() ) {
		$default = array( 'old_custom_fields' => array() );
		$args    = array_merge( $default, $args );
		...
		// Group Keys
		$general_group_field_keys      = array();
		$contact_group_info_field_keys = array();
		$map_group_field_keys          = array();
		$gallery_group_field_keys      = array();

		// General Group
		// ------------------------------
		// Title
		$general_group_field_keys[] = 'title';
		$preset_fields['title']     = array(
		...
		$preset_fields['description'] = array(
		...
		if ( ! empty( get_directorist_option( 'display_views_count', true ) ) ) {
			...
			$preset_fields['view_count'] = array(
				'type'           => 'number',
		...
		$preset_fields['hide_contact_owner'] = array(
			'type'         => 'checkbox',
		...
		if ( ! empty( get_directorist_option( 'display_social_info_field', true ) ) ) {
			...
			$preset_fields['social_info']    = array(
				'type'           => 'add_new',
				'field_key'      => 'social',
		...
		if ( ! empty( get_directorist_option( 'display_gallery_field', true ) ) ) {
			...
			$preset_fields['image_upload'] = array(
				'type'                  => 'media',
	...
	public function get_single_listings_contents_data( array $args = array() ) {
		...
		$single_listings_preset_fields = array();
		$map_group_field_keys          = array();
		$contact_info_group_field_keys = array();
		$video_group_field_keys        = array();

		$single_listings_preset_fields['tag'] = array(
			'icon'         => 'las la-tag',
			'widget_name'  => 'tag',
		...
		if ( ! empty( get_directorist_option( 'display_map_field', true ) ) ) {
			$map_group_field_keys[]               = 'map';
			$single_listings_preset_fields['map'] = array(
				'icon'         => 'las la-map',
				'widget_name'  => 'map',
				'widget_group' => 'preset_widgets',
			);
		...
		if ( ! empty( get_directorist_option( 'display_map_field', true ) ) ) {
			...
			$single_listings_preset_fields['address'] = array(
				'icon'                  => 'las la-map',
		...
		if ( ! empty( get_directorist_option( 'display_phone_field', true ) ) ) {
			...
			$single_listings_preset_fields['phone'] = array(
				'icon'         => 'las la-phone',
				'widget_name'  => 'phone',
				'widget_group' => 'preset_widgets',
			);
		...
		if ( ! empty( get_directorist_option( 'display_phone_field2', true ) ) ) {
			...
			$single_listings_preset_fields['phone2'] = array(
				'icon'         => 'las la-phone',
				'widget_name'  => 'phone2',
				'widget_group' => 'preset_widgets',
			);
		...
		if ( ! empty( get_directorist_option( 'display_zip_field', true ) ) ) {
			...
			$single_listings_preset_fields['zip'] = array(
		...
		if ( ! empty( get_directorist_option( 'display_email_field', true ) ) ) {
			...
			$single_listings_preset_fields['email'] = array(
		...
		if ( ! empty( get_directorist_option( 'display_website_field', true ) ) ) {
			...
			$single_listings_preset_fields['website'] = array(
				'icon'         => 'las la-globe',
				'widget_name'  => 'website',
				'widget_group' => 'preset_widgets',
			);
		...
		if ( ! empty( get_directorist_option( 'display_fax', true ) ) ) {
			...
			$single_listings_preset_fields['fax'] = array(
				'icon'         => 'las la-fax',
				'widget_name'  => 'fax',
				'widget_group' => 'preset_widgets',
			);
		...
		if ( ! empty( get_directorist_option( 'display_social_info_field', true ) ) ) {
			...
			$single_listings_preset_fields['social_info'] = array(
		...
		if ( ! empty( get_directorist_option( 'display_video_field', true ) ) ) {
			...
			$single_listings_preset_fields['video'] = array(
				'icon'         => 'las la-video',
				'widget_name'  => 'video',
				'widget_group' => 'preset_widgets',
			);
		...
		$single_listings_widgets_keys  = is_array( $single_listings_widgets ) ? array_keys( $single_listings_widgets ) : array();
		$single_listings_custom_fields = array();
		$custom_fields                 = $args['old_custom_fields'];
	...
	public function get_search_form_fields( array $args = array() ) {
		...
		$old_basic_search_fields = get_directorist_option( 'search_tsc_fields', array() );

		$search_form_fields_basic_items = array();
		...
		// Get Advanced Fields
		$old_advanced_search_fields        = get_directorist_option( 'search_more_filters_fields', array() );
		$search_form_fields_advanced_items = array();
		...
		if ( in_array( 'search_price', $old_advanced_search_fields ) || in_array( 'search_price_range', $old_advanced_search_fields ) ) {
			$search_form_fields_advanced_items['pricing'] = array(
				'label'                       => 'Pricing',
		...
		if ( in_array( 'search_custom_fields', $old_advanced_search_fields ) && ! empty( $args['old_custom_fields'] ) ) {
			...
			$custom_fields              = $args['old_custom_fields'];
			$submission_form_field_keys = array();
			...
			foreach ( $custom_fields as $field_key => $field_args ) {
				if ( empty( $field_args['searchable'] ) || ! in_array( $field_key, $submission_form_field_keys ) ) {
					continue; }

				$search_form_fields_advanced_items[ $field_key ] = array(
	...
	public function get_single_listing_header_data() {
		...
		$single_listing_header = array(
			'listings_header' => array(
	...
	public function get_listings_card_grid_view_data( array $args = array() ) {
	...
	// get_listings_card_grid_view_data
	public function get_listings_card_grid_view_with_thumbnail_data( array $args = array() ) {

		$default               = array( 'listings_card_wedgets' => $this->get_listings_card_wedgets_data() );
		$args                  = array_merge( $default, $args );
		$listings_card_wedgets = $args['listings_card_wedgets'];

		$card_layouts = array(
			'thumbnail_top_right'    => array(),
			'thumbnail_top_left'     => array(),
			'thumbnail_bottom_right' => array(),
			'thumbnail_bottom_left'  => array(),
			'thumbnail_avatar'       => array(),
			'body_top'               => array(),
			'body_bottom'            => array(),
			'body_excerpt'           => array(),
			'footer_right'           => array(),
		...
		$widget_layout_map = array(
			...
			'user_avatar'       => array(
				...
				'layout' => array( 'default' => 'thumbnail_avatar' ),
			),
			'listing_title'     => array(
				'enable' => get_directorist_option( 'display_title', true ),
				'layout' => array( 'default' => 'body_top' ),
			),
			'rating'            => array(
			...
			'listings_location' => array(
				'enable' => get_directorist_option( 'display_contact_info', true ),
				'layout' => array( 'default' => 'body_bottom' ),
			),
			'posted_date'       => array(
			...
			'excerpt'           => array(
				...
				'layout' => array( 'default' => 'body_excerpt' ),
			),
			'view_count'        => array(
				'enable' => get_directorist_option( 'display_view_count', true ),
				'layout' => array( 'default' => 'footer_right' ),
			),
			...
			'category'          => array(
				...
				'layout' => array( 'default' => 'footer_left' ),
			),
		);
		...
		foreach ( $widget_layout_map as $widget_key => $args ) {
			if ( isset( $args['enable'] ) && empty( $args['enable'] ) ) {
				continue; }
		...
		$listings_card_grid_view = apply_filters(
			'listings_card_grid_view_with_thumbnail',
			array(
				'thumbnail' => array(
	...
	public function get_listings_card_grid_view_without_thumbnail_data( array $args = array() ) {
		$default = array( 'listings_card_wedgets' => $this->get_listings_card_wedgets_data() );
		$args    = array_merge( $default, $args );
		...
		$listings_card_grid_view_body_quick_info = array();
		...
		$listings_card_grid_view_body_avatar = array();
		...
		$listings_card_grid_view_body_title = array();
		...
		$listings_card_grid_view_body_quick_actions = array();
		...
		$listings_card_grid_view_body_bottom = array();
		...
		$listings_card_grid_view_body_excerpt = array();
		...
		$listings_card_grid_view_footer_right = array();
		...
		$listings_card_grid_view_footer_left = array();
	...
	public function get_listings_card_list_view_data( array $args = array() ) {
	...
	public function get_listings_card_list_view_with_thumbnail_data( array $args = array() ) {
		$default = array( 'listings_card_wedgets' => array() );
		$args    = array_merge( $default, $args );

		$listings_card_wedgets = $args['listings_card_wedgets'];

		// $listings_card_list_view_thumbnail_top_right
		$listings_card_list_view_thumbnail_top_right = array();
		if ( get_directorist_option( 'display_feature_badge_cart', true ) ) {
		...
		$listings_card_list_view_body_top = array();
		if ( get_directorist_option( 'display_title', true ) ) {
		...
		$listings_card_list_view_footer_left = array();
		if ( get_directorist_option( 'display_category', true ) ) {
	...
	public function get_listings_card_list_view_without_thumbnail_data( array $args = array() ) {
		$default = array( 'listings_card_wedgets' => array() );
		$args    = array_merge( $default, $args );
	...
	public function get_listings_card_wedgets_data() {
		...
		$theme_user_avatar = array(
			'direo'    => array( 'align' => 'left' ),
			'dlist'    => array( 'align' => 'left' ),
			'dservice' => array( 'align' => 'left' ),
		);
	...
	public function get_old_custom_fields() {
		...
		foreach ( $old_fields as $old_field_id ) {
			...
			$accepted_types = array( 'text', 'number', 'date', 'color', 'time', 'radio', 'checkbox', 'select', 'textarea', 'url', 'file' );

			if ( ! in_array( $field_type, $accepted_types ) ) {
				continue; }
			...
			// field group
			$field_group = array( 'radio', 'checkbox', 'select' );
