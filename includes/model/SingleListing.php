...
class Directorist_Single_Listing {
	...
	public function build_content_data() {
		...
		if ( ! empty( $single_fields['fields'] ) ) {
			foreach ( $single_fields['fields'] as $key => $value ) {
				...
				$single_fields['fields'][ $key ]['options']   = array();
	...
	public function get_field_value( $data = array() ) {
		$post_id = $this->id;
	...
	public function contact_owner_fields( $field_data = array() ) {
	...
	public function author_display_email( $section_data = array() ) {
	...
	public function get_related_listings( $data = array() ) {
		...
		$related = new Directorist_Listings( array(), 'related', $args, array( 'cache' => false ) );
