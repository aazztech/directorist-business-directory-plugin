...
class Directorist_Listings {
	...
	public function __construct( $atts = array(), $type = 'listing', $query_args = false, array $caching_options = array() ) {
		...
		} elseif ( $this->type == 'search_result' || $this->type == 'instant_search' || ! empty( $_GET ) ) {
	...
	public function parse_search_query_args() {
		...
		if ( 'address' == $this->radius_search_based_on && ! empty( $_REQUEST['miles'] ) && ! empty( $_REQUEST['address'] ) && ! empty( $_REQUEST['cityLat'] ) && ! empty( $_REQUEST['cityLng'] ) ) {
			$distance = directorist_get_distance_range( $_REQUEST['miles'] );           $args['atbdp_geo_query'] = array(
	...
	public function setup_loop( array $args = array() ) {
		$default  = array(
			'template' => 'grid',
		);
	...
	public function is_custom_field( $data ) {
		$fields      = array( 'checkbox', 'color_picker', 'date', 'file', 'number', 'radio', 'select', 'text', 'textarea', 'time', 'url' );
	...
	public function display_search_button() {
		...
		if ( in_array( $this->options['all_listing_layout'], array( 'right_sidebar', 'left_sidebar' ) )
