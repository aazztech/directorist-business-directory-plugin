...
class ATBDP_Upgrade {
	...
	public $directorist_notices = array();

	public $directorist_migration = array();
	...
	public function __construct() {
		...
		add_action( 'admin_init', array( $this, 'v8_force_migration' ) );
	}
	...
	public function run_v8_migration() {
		...
		if ( did_action( 'elementor/loaded' ) && ! file_exists( $path ) ) {
			...
			directorist_download_plugin( array( 'url' => 'https://downloads.wordpress.org/plugin/addonskit-for-elementor.zip' ) );
		...
		foreach ( $directory_types as $directory_type ) {
			...
			if ( $description ) {
				...
				$contents['fields']['description'] = array(
					'icon'                => 'las la-tag',
	...
	private function search_field_label_migration( $directory_id ) {
		...
		$search_fields = is_array( $search_fields ) ? $search_fields : array();
		$fields        = isset( $search_fields['fields'] ) && is_array( $search_fields['fields'] ) ? $search_fields['fields'] : array();
	...
	public function v8_extension_upgrade_notice( $outdated_extensions = array() ) {
		if ( ! self::can_manage_plugins() ) {
			return;
		}
