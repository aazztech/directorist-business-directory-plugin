...
foreach ( $style_component as $style_prop => $style_value ) {
	$style .= sprintf('%s: %s;', $style_prop, $style_value);
}
...
// Card Contain
$card_contain_wrap = sprintf("<div class='directorist-thumnail-card directorist-card-contain' style='%s'>", $style);
$image_contain_html = $card_contain_wrap . $blur_bg . $front_wrap_html . "</div>";

// Card Cover
$card_cover_wrap = sprintf("<div class='directorist-thumnail-card directorist-card-cover' style='%s'>", $style);
$image_cover_html = $card_cover_wrap . $front_wrap_html . "</div>";

// Card Full
$card_full_wrap = sprintf("<div class='directorist-thumnail-card directorist-card-full' style='%s'>", $style);
