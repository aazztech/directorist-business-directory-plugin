...
class Helper {
	...
	public static function is_legacy_mode(): bool {
		return false;
	}
	...
	public static function get_first_wp_error_message( $wp_error ) {
		...
		$error_key  = ( $error_keys === [] ) ? '' : $error_keys[0];

		return ( $error_key !== 0 && ($error_key !== '' && $error_key !== '0') && is_array( $wp_error->errors[ $error_key ] ) && (isset($wp_error->errors[ $error_key ]) && $wp_error->errors[ $error_key ] !== []) ) ? $wp_error->errors[ $error_key ][0] : '';
	}
	...
	public static function update_listings_ratings_meta( $post_id = 0 ): bool {
		...
		foreach ( $reviews as $review ) {
			...
			$total_ratings += ( float ) $review[ 'rating' ];
		}
	...
	public static function listing_price( $id='' ): void {
		if ( !$id ) {
			$id = get_the_ID();
		}
	...
	public static function pricing_type( $listing_id ) {
		return get_post_meta( $listing_id, '_atbd_listing_pricing', true );
	}
	...
	public static function has_price_range( $listing_id ) {
		return get_post_meta( $listing_id, '_price_range', true );
	}

	public static function price_template( $listing_id ): void {
		$price = get_post_meta( $listing_id, '_price', true );
		self::get_template( 'global/price', ['price' => $price] );
	}

	public static function price_range_template( $listing_id ): void {
		...
		self::get_template( 'global/price-range', ['active_items' => $active_items, 'currency' => $currency, 'price_range_text' => $price_range_text] );
	}

	public static function formatted_price( $price ): string {
		...
		return $before . atbdp_format_amount( $price, $allow_decimal ) . $after;
	}
	...
	public static function phone_link( $args ): string {
		$args = array_merge( [
			'number'    => '',
			'whatsapp'  => false,
		], $args );
	...
	public static function parse_video( $url ): string {
		$embeddable_url = '';
	...
	public static function is_popular( $listing_id ): bool {
	...
	public static function badge_exists( $listing_id ): bool {
		// @cache @kowsar
	...
	public static function is_new( $listing_id ): bool {
	...
	public static function multi_directory_enabled(): bool {
	...
	public static function default_preview_image_src( $directory_id ) {
		...
		if ( ! empty( $settings['preview_image'] ) ) {
			...
			$default_preview = $default_img ?: DIRECTORIST_ASSETS . 'images/grid.jpg';
		}
	...
	public static function single_listing_dummy_shortcode( $shortcode, $atts = [] ): string {
		$atts_string = '';
	 ...
	 * @return array
	 */
	public static function get_directory_types_with_custom_single_page( $page_id = null ): ?array {
		$args = [
			'meta_query' => [
				'page_enabled' => [
		...
		return array_filter( $directory_types, static function( $directory_type ) use ( $page_id ) {
     ...
     * @return mixed[]
     */
    public static function builder_selected_single_pages(): array {
		// @cache @kowsar
		$pages = [];
	...
	public static function get_listing_payment_status( $listing_id = '' ) {
		...
		return get_post_meta( $order_id, '_payment_status', true );
	}
	...
	public static function get_listing_order_id( $listing_id = '' ) {
		...
		return ( $orders->have_posts() ) ? $orders->post->ID : '';
	}

	public static function add_hidden_data_to_dom( string $data_key = '', array $data = [] ): void {
	...
	public static function add_shortcode_comment( string $shortcode = '' ): void {
	...
	public static function sanitize_query_strings( $url = '' ) {
		...
		$qs = ( $matches === [] ) ? '' : ltrim( $matches[0], '?' );
		$qs = ( $qs === '' || $qs === '0' ) ? '' : '?' . str_replace( '?', '&', $qs );

		return preg_replace( $qs_pattern, $qs, $url );
	}
     ...
     */
    public static function validate_date_format( $date, $format = 'Y-m-d h:i:s' ): bool {
	 ...
	 * @return string URL
	 */
	public static function escape_query_strings_from_url( $url = '' ): ?string {
		$matches = [];
		$qs_pattern = '/[?].+/';
	 ...
	 */
	public static function get_query_string_pattern(): string {
		return '/\/?[?].+\/?/';
	}
	 ...
	 * @return string URL
	 */
	public static function join_slug_to_url( $url = '', $slug = '' ): ?string {
		if ( empty( $url ) ) {
			return $url;
		}
	 ...
	 */
	public static function get_query_strings_from_url( $url = '' ): ?string {
		if ( empty( $url ) ) {
			return $url;
		}
