...
if ( ! class_exists( 'ATBDP_SEO' ) ) :
    ...
    class ATBDP_SEO {
         ...
         * @since 7.0.8
         */
        public function setup_seo(): void {
			...
			add_filter( 'the_title', [ $this, 'update_taxonomy_page_title' ], 10, 2 );
			add_filter( 'single_post_title', [ $this, 'update_taxonomy_single_page_title' ], 10, 2 );
			add_filter( 'pre_get_document_title', [$this, 'atbdp_custom_page_title'], 10 );

			add_action('wp_head', [$this, 'atbdp_add_meta_keywords'], 10, 2);
			add_filter('wp_title', [$this, 'atbdp_custom_page_title'], 10, 2);
			add_action('wp_head', [$this, 'add_opengraph_meta'], 10, 2);
			add_action('wp_head', [$this, 'add_texonomy_canonical']);
			add_action( 'wp', [ $this, 'remove_duplicate_canonical' ] );
		}

		// Add Yoast Compatibility
        public function add_yoast_compatibility(): void {
            add_filter( 'the_title', [ $this, 'update_taxonomy_page_title' ], 10, 2 );
			add_filter( 'single_post_title', [ $this, 'update_taxonomy_single_page_title' ], 10, 2 );
			add_filter( 'pre_get_document_title', [$this, 'atbdp_custom_page_title'], 10 );
			...
			add_filter('wpseo_canonical', [$this, 'directorist_canonical']);
			add_filter('wpseo_opengraph_url', [$this, 'directorist_canonical']);
        ...
        public function add_rankmath_compatibility(): void {
            add_filter( 'the_title', [ $this, 'update_taxonomy_page_title' ], 10, 2 );
			add_filter( 'single_post_title', [ $this, 'update_taxonomy_single_page_title' ], 10, 2 );
			add_filter( 'pre_get_document_title', [$this, 'atbdp_custom_page_title'], 10 );
        ...
        public function optimize_rankmath_frontend_meta_title( $content ) {
			...
			return str_replace( $current_page_title, $term['name'], $content );
        }
        ...
        public function yoast_sitemap_exclude_taxonomy( $value, $taxonomy ): ?bool {
        ...
        public function get_taxonomy_term($id = null)
        {
            ...
            if ($post instanceof WP_Post && ($post->post_type == 'post' || $post->post_type == 'page')) {
                ...
                if ($post->ID == $LOC_page_ID && $slug = get_query_var('atbdp_location')) {
        ...
        public function get_taxonomy_page_title( $default_title, $page_id )
        {
            ...
            return ( empty( $term ) ) ? $default_title : $term->name;
        }
        ...
        public function get_taxonomy_page_term_data( $page_id ) {
            ...
            return ( $term_data ) ? json_decode( json_encode( $term_data ), true ) : [];
        }
        ...
        public function wpseo_metadesc($desc)
        {
            ...
            $desc_template = '';
            if (is_singular('at_biz_dir') && !empty($overwrite_yoast)) {
                return '';
            }
        ...
        public function remove_duplicate_canonical(): void {
            global $post;
        ...
        public function add_texonomy_canonical(): void
        {
            global $post;
        ...
        public function atbdp_add_meta_keywords(): void
        {
        ...
        // add_opengraph_meta
        public function add_opengraph_meta(): void {
            ...
            foreach ( $seo_meta_data as $meta_key => $meta_value ) {
                if ( isset($og_metas[ $meta_key ]) && $og_metas[ $meta_key ] !== [] ) {
                    $og_metas[ $meta_key ]['content'] = $seo_meta_data[ $meta_key ];
                }
            }

            // Adjust the title
            if ( ! empty( $seo_meta_data['site_name'] ) && (isset($og_metas['title']) && $og_metas['title'] !== []) ) {
            ...
            foreach ( $og_metas as $meta_attributes ) {
                $props = '';
        ...
        public function get_home_page_seo_meta( $default_seo_meta = [] ) {
            ...
            return ( is_array( $default_seo_meta ) ) ? array_merge( $default_seo_meta, $seo_meta ) : $seo_meta;
        }
        ...
        public function get_search_result_page_seo_meta( $default_seo_meta = [] ) {
            ...
            $in_s_string_text = $query === '' || $query === '0' ? '' : $query;
            $in_cat_text      = empty( $category ) ? '' : ( $query === '' || $query === '0' ? '' : __( 'from', 'directorist ') ) . $category->name;
            $in_loc_text      = empty( $location ) ? '' : ( $query === '' || $query === '0' ? '' : __( 'from', 'directorist ') ) . $location->name;

            $how_to = get_directorist_option('meta_title_for_search_result', 'searched_value');

            if ( 'searched_value' === $how_to && ($query !== '' && $query !== '0' || ! empty( $category ) || ! empty( $location )) ) {
        ...
        public static function get_directorist_current_page(): string {
