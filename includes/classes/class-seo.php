...
if ( ! class_exists( 'ATBDP_SEO' ) ) :
	...
	class ATBDP_SEO {
		...
		public function __construct() {
			...
			add_action( 'init', array( $this, 'setup_seo' ) );
		}
		...
		public function add_yoast_compatibility() {
			...
			add_filter( 'wpseo_sitemap_exclude_taxonomy', array( $this, 'yoast_sitemap_exclude_taxonomy' ), 10, 2 );
		}
		...
		public function add_rankmath_compatibility() {
			...
			add_filter( 'rank_math/frontend/title', array( $this, 'optimize_rankmath_frontend_meta_title' ), 20, 1 );
			add_filter( 'rank_math/frontend/description', array( $this, 'optimize_rankmath_frontend_meta_description' ), 20, 1 );
			add_filter( 'rank_math/frontend/canonical', array( $this, 'directorist_canonical' ), 20, 1 );
		}
		...
		public function yoast_sitemap_exclude_taxonomy( $value, $taxonomy ) {
			$taxonomy_to_exclude = array( ATBDP_CATEGORY, ATBDP_LOCATION, ATBDP_TAGS, ATBDP_DIRECTORY_TYPE );
		...
		public function get_taxonomy_page_title( $default_title, $page_id ) {
			...
			if ( ! in_array( $page_id, array( $category_page_id, $location_page_id, $tag_page_id ) ) ) {
				return $default_title;
			}
		...
		public function get_taxonomy_page_term_data( $page_id ) {
			...
			$term_pages = array( $category_page_id, $location_page_id, $tag_page_id );
			...
			$term_data = ( $term_data ) ? json_decode( json_encode( $term_data ), true ) : array();
		...
		public function remove_duplicate_canonical() {
			...
			$targeted_pages = array( $CAT_page_ID, $LOC_page_ID, $Tag_page_ID );
			if ( ! in_array( $post->ID, $targeted_pages ) ) {
				return;
			}
		...
		public function get_seo_meta_data() {
			...
			if ( method_exists( $this, $callback ) ) {
				$seo_meta = call_user_func_array( array( $this, $callback ), array( $seo_meta ) );
			}
		...
		public function get_home_page_seo_meta( $default_seo_meta = array() ) {
			$seo_meta         = array();
			$seo_meta['page'] = 'home';
		...
		public function get_search_result_page_seo_meta( $default_seo_meta = array() ) {
			$seo_meta         = array();
		...
		public function get_add_listing_page_seo_meta( $default_seo_meta = array() ) {
			$seo_meta         = array();
		...
		public function get_all_listing_page_seo_meta( $default_seo_meta = array() ) {
			$seo_meta         = array();
		...
		public function get_dashboard_page_seo_meta( $default_seo_meta = array() ) {
			$seo_meta         = array();
		...
		public function get_author_page_seo_meta( $default_seo_meta = array() ) {
			$seo_meta         = array();
		...
		public function get_category_page_seo_meta( $default_seo_meta = array() ) {
			$seo_meta         = array();
		...
		public function get_all_locations_page_seo_meta( $default_seo_meta = array() ) {
			$seo_meta         = array();
		...
		public function get_single_listing_page_seo_meta( $default_seo_meta = array() ) {
			$seo_meta         = array();
			$seo_meta['page'] = 'all_locations';
		...
		public function get_single_category_page_seo_meta( $default_seo_meta = array() ) {
			global $post;
			...
			if ( $post->ID == $CAT_page_ID && $slug && ! empty( $term ) ) {
				if ( ! empty( $term->description ) ) {
					$seo_meta['description'] = $term->description;
				}
		...
		public function get_single_location_page_seo_meta( $default_seo_meta = array() ) {
			global $post;
		...
		public function get_single_tag_page_seo_meta( $default_seo_meta = array() ) {
			global $post;
		...
		public function get_registration_page_seo_meta( $default_seo_meta = array() ) {
			$seo_meta         = array();
			$seo_meta['page'] = 'all_locations';
		...
		public function get_login_page_seo_meta( $default_seo_meta = array() ) {
			$seo_meta         = array();
			$seo_meta['page'] = 'login';
		...
		public function sync_with_yoast_seo_meta( array $args = array() ) {
			$default = array(
				'url'      => '',
				'seo_meta' => array(),
			);
